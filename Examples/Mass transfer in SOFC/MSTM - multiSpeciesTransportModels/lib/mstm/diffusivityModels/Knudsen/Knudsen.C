...
:
    transportDic_(transportDic),
    
    thermoDic_(thermoDic),
    
    name_(name),

    T_(T)
{  
    d = dimensionedScalar(transportDic_.lookup("poreDiametre")).value();
  
    const specie specie(thermoDic_.lookup(name));
    W = specie.W();
    RR = specie.RR;
}
...
) const
{
    ...
    forAll(T, facei)
    {
        dk[facei] = d / 3 * sqrt((8*RR*T[facei])/(3.14159*W));
    }
...
Foam::tmp<Foam::volScalarField> Foam::Knudsen::DK() const
{
    ...
    {
	dk[celli] = d / 3 * sqrt((8*RR*T_[celli])/(3.14159*W));
    }
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        forAll(pT, facei)
        {
            pDk[facei] = d / 3 * sqrt((8*RR*T_[facei])/(3.14159*W));
        }
    }
