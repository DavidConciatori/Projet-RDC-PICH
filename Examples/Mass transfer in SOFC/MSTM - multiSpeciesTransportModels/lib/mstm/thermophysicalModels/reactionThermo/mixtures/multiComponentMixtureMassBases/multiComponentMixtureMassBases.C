...
#include "multiComponentMixtureMassBases.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

template<class ThermoType>
const ThermoType& Foam::multiComponentMixtureMassBases<ThermoType>::constructSpeciesData
(
...
template<class ThermoType>
void Foam::multiComponentMixtureMassBases<ThermoType>::constructMixtureComposition
(
    const fvMesh& mesh
)
{
...
template<class ThermoType>
Foam::multiComponentMixtureMassBases<ThermoType>::multiComponentMixtureMassBases
(
    const dictionary& thermoDict,
...
template<class ThermoType>
void Foam::multiComponentMixtureMassBases<ThermoType>::correct()
{ 
    // Update the molar weight of the mixture
    scalarField& WCells = W_.internalField();
    forAll(WCells, celli)
    {
        WCells[celli] = cellMixture(celli).W();
    }
    forAll(W_.boundaryField(), patchi)
    {
        ...
        forAll(pW, facei)
        {
            pW[facei] = patchFaceMixture(patchi, facei).W();
        }
    }
 
    // Update the molar fractions
    forAll(x_, speciesI)
    {        
        const scalarField& yCells = y_[speciesI].internalField();
        const scalarField& WCells = W_.internalField();            
        scalarField& xCells = x_[speciesI].internalField();
        forAll(xCells, celli)
        {
            xCells[celli] =
                yCells[celli] * WCells[celli] / speciesData_[speciesI].W();
        }
        forAll(x_[speciesI].boundaryField(), patchi)
        {
            const fvPatchScalarField& py =
                y_[speciesI].boundaryField()[patchi];
            const fvPatchScalarField& pW = W_.boundaryField()[patchi];            
            fvPatchScalarField& px = x_[speciesI].boundaryField()[patchi];
            forAll(px, facei)
            {
                px[facei] =
                    py[facei] * pW[facei] / speciesData_[speciesI].W();
            }
        }
    }
}
...
template<class ThermoType>
void Foam::multiComponentMixtureMassBases<ThermoType>::correctMolarFluxes()
{ 
    forAll(N_, i)
    {                
        ...
        { 
            forAll(N_[i].boundaryField()[boundaryI], faceI)
            {
                ...
                n_[i].boundaryField()[boundaryI][faceI] / speciesData_[i].W();
            }
        }
    }
}
...
template<class ThermoType>
const ThermoType& Foam::multiComponentMixtureMassBases<ThermoType>::cellMixture
(
    const label celli
) const
{
...
template<class ThermoType>
const ThermoType& Foam::multiComponentMixtureMassBases<ThermoType>::patchFaceMixture
(
    const label patchi,
    const label facei
) const
...
template<class ThermoType>
void Foam::multiComponentMixtureMassBases<ThermoType>::read
(
    const dictionary& thermoDict
)
{
